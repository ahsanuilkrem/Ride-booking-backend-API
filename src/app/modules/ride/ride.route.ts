...
router.get( "/rideHistory/:rideId", checkAuth(Role.RIDER), rideControler.getRideById);
router.patch("/cancel/:rideId", checkAuth(Role.RIDER), rideControler.cancelRide)
router.patch("/rideStatus/:rideId", checkAuth(Role.ADMIN, Role.DRIVER), rideControler.updateRideStatus)
router.get("/earnings", checkAuth(Role.DRIVER), rideControler.viewEarningHistory);
