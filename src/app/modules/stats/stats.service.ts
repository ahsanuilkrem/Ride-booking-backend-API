...
import { RideStatus } from "../ride/ride.interfaces";
import { availaStatus, IsStatus, VEHICLE_TYPE } from "../driver/driver.interfaces";
import { IsActive,  Role } from "../user/user.interfaces";
...
const getPublicHomepageStats = async () => {
  const [
    ...
    Driver.countDocuments({ status: IsStatus.APPROVED }),
...
const getUserStats = async () => {
  const [
    ...
    User.countDocuments({ role: { $in: [Role.ADMIN] } }),
...
const getDriverStats = async () => {
  const [
    ...
    Driver.countDocuments(),
    Driver.countDocuments({ status: IsStatus.APPROVED }),
    Driver.countDocuments({ status: IsStatus.PENDING }),
    Driver.countDocuments({ status: IsStatus.REJECTED }),
    Driver.countDocuments({ status: IsStatus.SUSPEND }),
    Driver.countDocuments({ availability: availaStatus.AVAILABLE }),
    Driver.countDocuments({ availability: availaStatus.UNAVAILABLE }),
    Driver.countDocuments({ availability: availaStatus.ON_TRIP }),
    Driver.aggregate([
